...
console.log(chalk.green('\n(2) Forks and Reordering\n'))
{
  // Create two chat users, each with their own Hypercores.
  const st = new Corestore(ram);
  const userA = st.get({name: 'userA'});
  const userB = st.get({name: 'userB'});
...
console.log(chalk.green('\n(2) Locking Forks in Time\n'))
{
  ...
  await baseA.append('A*: Yeeeeeh... both online!')
  await baseB.append('B*: didididididid both online!')
  await baseA.append('A$: puff puff puff....')
  ...
  await baseA.append('A$: neneneneneenen', [])
